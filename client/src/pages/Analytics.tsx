...
export default function Analytics() {
  ...
  return (
    <div className="min-h-screen bg-paper px-6 py-8 max-w-4xl mx-auto">
      ...
      <h1 className="text-xl font-bold text-champagne mb-6">Exam Analytics</h1>
      ...
      {data.submissionCount === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-8">
            <Card className="p-4"><div className="text-xs text-bronze">SUBMISSIONS</div><div className="text-lg font-bold text-champagne">{data.submissionCount}</div></Card>
            <Card className="p-4"><div className="text-xs text-bronze">AVERAGE</div><div className="text-lg font-bold text-champagne">{data.averageScore}</div></Card>
            <Card className="p-4"><div className="text-xs text-bronze">HIGHEST</div><div className="text-lg font-bold text-champagne">{data.highestScore}</div></Card>
            <Card className="p-4"><div className="text-xs text-bronze">LOWEST</div><div className="text-lg font-bold text-champagne">{data.lowestScore}</div></Card>
            <Card className="p-4"><div className="text-xs text-bronze">PASS %</div><div className="text-lg font-bold text-champagne">{data.passPercentage}%</div></Card>
          </div>

          <Card className="p-6 mb-8">
            <div className="font-semibold text-champagne mb-4">Question-wise accuracy</div>
            <Bar data={chartData} options={{ responsive: true, scales: { y: { min: 0, max: 100 } } }} />
          </Card>

          <Card className="p-6">
            <div className="font-semibold text-champagne mb-4">Most difficult questions</div>
            <div className="space-y-2">
              {[...data.questionAccuracy].sort((a, b) => a.accuracyPercent - b.accuracyPercent).slice(0, 5).map((q, i) => (
                <div key={q.questionId} className="flex items-center justify-between text-sm border-b border-gray-100 pb-2">
                  <span className="text-champagne">{q.text}</span>
