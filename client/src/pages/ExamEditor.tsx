  ...
  return (
    <div className="min-h-screen bg-paper px-6 py-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-ink">{isNew ? "Create Exam" : "Edit Exam"}</h1>
          <p className="text-sm text-gray-500">Question images sit right above the options</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="p-6 lg:col-span-1 h-fit">
          <div className="font-semibold text-ink mb-4">Exam details</div>
          <label className="text-xs font-semibold text-gray-500">Title</label>
          <input value={exam.title} onChange={(e) => setExam((x) => ({ ...x, title: e.target.value }))}
            className="w-full rounded-lg px-3 py-2 text-sm mt-1 mb-3 border border-gray-300" />
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <label className="text-xs font-semibold text-gray-500">Duration (min)</label>
              <input type="number" value={exam.durationMinutes} onChange={(e) => setExam((x) => ({ ...x, durationMinutes: Number(e.target.value) }))}
                className="w-full rounded-lg px-3 py-2 text-sm mt-1 border border-gray-300" />
            </div>
            <div>
          ...
          <label className="text-xs font-semibold text-gray-500">&minus; Negative marks</label>
          <input type="number" value={exam.defaultNegativeMarks} onChange={(e) => setExam((x) => ({ ...x, defaultNegativeMarks: Number(e.target.value) }))}
            className="w-full rounded-lg px-3 py-2 text-sm mt-1 mb-3 border border-gray-300" />

          <label className="text-xs font-semibold text-gray-500">Passing marks</label>
          ...
          <textarea rows={4} value={exam.instructions} onChange={(e) => setExam((x) => ({ ...x, instructions: e.target.value }))}
            className="w-full rounded-lg px-3 py-2 text-sm mt-1 mb-4 border border-gray-300" />

          <Button className="w-full" onClick={saveExamMeta} disabled={saving}>{saving ? "Saving…" : "Save Exam Details"}</Button>

          {savedExamId && <div className="text-xs text-gray-400 mt-3">{questions.length} question(s) added</div>}
        </Card>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Card className="p-6">
            <div className="font-semibold text-ink mb-1">{editingId ? "Editing question" : "Add a question"}</div>
            <div className="text-xs text-gray-500 mb-4">Upload a diagram or figure — it appears above the four options.</div>

            <label className="rounded-xl flex flex-col items-center justify-center gap-2 py-6 mb-4 cursor-pointer border-2 border-dashed border-gray-300 bg-gray-50">
              {uploading ? (
                <span className="text-xs text-gray-500">Uploading…</span>
              ) : form.imageUrl ? (
                <img src={form.imageUrl} alt="" className="max-h-40 rounded-lg" />
              ) : (
                <span className="text-xs text-gray-500">Click to upload question image (optional)</span>
              )}
            ...
            <textarea rows={3} placeholder="Type the question here…" value={form.text}
              ...
              className="w-full rounded-lg px-3 py-2 text-sm mb-4 border border-gray-300" />

            <div className="space-y-2 mb-4">
              {form.options.map((opt: string, i: number) => (
                <div key={i} className="flex items-center gap-2">
                  <button onClick={() => setForm((f: any) => ({ ...f, correctOptionIndex: i }))}
                    className={`rounded-full w-7 h-7 flex items-center justify-center text-xs font-bold shrink-0 ${form.correctOptionIndex === i ? "bg-green-600 text-white" : "bg-gray-100 text-gray-600"}`}>
                    {"ABCD"[i]}
                  </button>
                  <input value={opt} onChange={(e) => updateOption(i, e.target.value)} placeholder={`Option ${"ABCD"[i]}`}
                    className={`flex-1 rounded-lg px-3 py-2 text-sm border ${form.correctOptionIndex === i ? "border-green-500" : "border-gray-300"}`} />
                </div>
              ))}
            </div>
          ...
          {questions.length > 0 && (
            <Card className="p-6">
              <div className="font-semibold text-ink mb-4">Question bank ({questions.length})</div>
              <div className="space-y-3">
                {questions.map((q, i) => (
                  <div key={q._id} className="flex items-start justify-between gap-3 p-3 rounded-xl bg-gray-50">
                    <div className="flex gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-sm text-ink truncate max-w-sm">{q.text}</div>
                      </div>
                    </div>
                    <div className="flex gap-1.5 shrink-0">
                      <button onClick={() => editQuestion(q)} className="rounded-full p-2 bg-white border border-gray-300 text-xs">Edit</button>
