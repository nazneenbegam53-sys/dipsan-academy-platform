      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="p-6 lg:col-span-1 h-fit">
          ...
          <input value={exam.title} onChange={(e) => setExam((x) => ({ ...x, title: e.target.value }))}
            className="w-full rounded-sm px-3 py-2 text-sm bg-charcoal text-ink mt-1 mb-3 border border-ink/15" />
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <input type="number" value={exam.durationMinutes} onChange={(e) => setExam((x) => ({ ...x, durationMinutes: Number(e.target.value) }))}
                className="w-full rounded-sm px-3 py-2 text-sm bg-charcoal text-ink mt-1 border border-ink/15" />
            </div>
            <div>
          ...
          <textarea rows={4} value={exam.instructions} onChange={(e) => setExam((x) => ({ ...x, instructions: e.target.value }))}
            className="w-full rounded-sm px-3 py-2 text-sm bg-charcoal text-ink mt-1 mb-4 border border-ink/15" />
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <Card className="p-6">
            ...
            <label className="rounded-xl flex flex-col items-center justify-center gap-2 py-6 mb-4 cursor-pointer border-2 border-dashed border-ink/15 bg-ink/60">
              {uploading ? (
            ...
            <textarea rows={3} placeholder="Type the question here…" value={form.text}
              ...
              className="w-full rounded-sm px-3 py-2 text-sm bg-charcoal text-ink mb-4 border border-ink/15" />

            <div className="space-y-2 mb-4">
              {form.options.map((opt: string, i: number) => (
                <div key={i} className="flex items-center gap-2">
                  <button onClick={() => setForm((f: any) => ({ ...f, correctOptionIndex: i }))}
                    className={`rounded-full w-7 h-7 flex items-center justify-center text-xs font-bold shrink-0 ${form.correctOptionIndex === i ? "bg-green-600 text-white" : "bg-paper border border-ink/10 text-bronze"}`}>
                    {"ABCD"[i]}
                  </button>
                  <input value={opt} onChange={(e) => updateOption(i, e.target.value)} placeholder={`Option ${"ABCD"[i]}`}
                    className={`flex-1 rounded-sm px-3 py-2 text-sm bg-charcoal text-ink border ${form.correctOptionIndex === i ? "border-green-500" : "border-ink/15"}`} />
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <input placeholder="Chapter" value={form.chapter} onChange={(e) => setForm((f: any) => ({ ...f, chapter: e.target.value }))}
                className="rounded-sm px-3 py-2 text-sm bg-charcoal text-ink border border-ink/15" />
                ...
                {questions.map((q, i) => (
                  <div key={q._id} className="flex items-start justify-between gap-3 p-3 rounded-xl bg-ink/60">
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      <button onClick={() => editQuestion(q)} className="rounded-full p-2 bg-charcoal border border-ink/15 text-xs">Edit</button>
