    ...
    return (
      <div className="min-h-screen bg-paper px-6 py-8 max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-3 mb-7">
          ...
          <Card className="p-4 bg-paper border border-white/10 border-none"><div className="text-xs text-bronze">UNATTEMPTED</div><div className="text-xl font-bold text-bronze">{detail.attempt.unattemptedCount}</div></Card>
        </div>
          ...
          {detail.exam.questions.map((q, i) => {
            ...
            return (
              <Card key={q._id} className="p-5">
                ...
                {q.imageUrl && <img src={q.imageUrl} alt="" className="max-h-56 rounded-lg mb-3 border border-white/10" />}
                ...
                {(q.explanation || q.explanationImageUrl) && (
                  <div className="mt-3 rounded-xl border-l-4 border-gold bg-gold/15 px-3 py-2 text-xs text-mist">
                    ...
                    {q.explanationImageUrl && (
                      <img src={q.explanationImageUrl} alt="Solution" className="mt-2 max-h-48 rounded-lg border border-white/10" />
                    )}
                  </div>
                )}
              </Card>
              ...
              {attempts.map((a) => {
                ...
                return (
                  <tr key={a._id} className="border-t border-white/10">
                    ...
                    <td className="px-4 py-3"><button onClick={() => openDetail(a._id)} className="text-xs font-semibold text-mist">View →</button></td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="fixed inset-0 flex items-center justify-center px-6 z-50 bg-black/50">
          <Card className="p-7 w-full max-w-md max-h-[80vh] overflow-auto">
            <div className="flex items-center justify-between mb-4">
              <div className="font-bold text-mist">Violation report</div>
              ...
              <div className="space-y-3">
                {violationReport.map((r, i) => (
                  <div key={i} className="text-sm border-b border-white/10 pb-2">
                    <div className="font-semibold text-mist">{r.student.name} — {r.violationCount} violation(s)</div>
